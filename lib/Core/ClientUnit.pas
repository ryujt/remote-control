...
{$ENDIF}
  ...
  FConnectionID := FJsonData.Integers['id'];
...
begin
  FIsFirstScreen := true;
  FSocket.Connect(Gateway_Host, Gateway_Port);
end;
...
begin
  case TFrameType(APacket^.PacketType) of
    ...
    ftFrameEnd: FDeskUnZip.Execute(APacket);
  end;
end;
...
begin
  case TPacketType(APacket^.PacketType) of
    ptErPeerConnected: TCore.Obj.View.sp_ErPeerConnected;
    ptPeerConnected: begin
      TCore.Obj.View.sp_PeerConnected;
      FDeskZip.Prepare(0);
    end;
    ptPeerDisconnected: TCore.Obj.View.sp_PeerDisconnected;
